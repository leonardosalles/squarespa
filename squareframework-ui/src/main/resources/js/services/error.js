define(['square/services/services',
        'text!square/views/stacktrace.html'],
	...
	services.provider('$error', function () {
		...
		var template = '<div class="modal modal-error hide fade" tabindex="-1" data-backdrop="static" data-keyboard="false">' +
							'<div class="modal-header">' +
								'<h3>{{\'square.exception.error\' | i18n}}</h3>' +
							'</div>' +
		...
		var templateReady = '<div class="modal modal-error hide fade" tabindex="-1">' +
								'<div class="modal-header">' +
									'<h3>{{\'square.exception.error\' | i18n}}</h3>' +
								'</div>' +
		...
		var build = function (response) {
			...
			case 408:
			case 422:
				return {message: I18n.t('square.exception.code.' + response.status.toString())};
				
			default:
				return {message: I18n.t('square.exception.code.500')};
			}
		};
