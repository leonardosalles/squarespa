			...
			function Pager(resource, array) {
				...
				this._goTo = function (page, flag) {
					...
					this.resource.query(actualFilter, function (data, headers) {
						self.results = data;
						self.totalResults = parseInt(headers('x-square-meta-total-count'), 10);
						self.numPages =  parseInt(headers('x-square-meta-total-pages'), 10);
						self.currentLimit = parseInt(headers('x-square-meta-current-limit'), 10);
						self.currentPage = parseInt(headers('x-square-meta-current-page'), 10);
						...
						if (flag && self.totalResults === 0) {
							$notify.info(I18n.t('square.datascroller.nomatches'));
						}
