define(['square/directives/directives'], function (directives) {
	...
	var select2 =  function () {
		...
		return {
			...
			link: function (scope, element, attr, ctrl) {
				if (attr.squareDuallistbox || element.data('select2')) {
					return;
				}
				...
				var opts = angular.extend({placeholder: attr.placeholder || ' ', allowClear: true, width: 'off'}, scope.$eval(attr.squareSelect2));
				if (!ctrl) {
				...
				var focus = function () {
					...
					if ($(container).hasClass('ng-pristine') && !$(container).hasClass('select2-container-active')) {
						$(container).tooltip('destroy');
					} else {
				...
				ctrl.$formatters.push(function (value) {
					return (angular.isArray(value) || value === 0 || value === '0') ? value : value || '';
				});

				ctrl.$parsers.push(function (value) {
					return (angular.isArray(value) || value === 0 || value === '0') ? value : value || null;
				});
				...
				if (element.is('select')) {
					...
					scope.$watch(function () {
						if (element.data('select2')) {
							...
							element.select2('val', (angular.isArray(value) || value === 0 || value === '0') ? value : value || null);
						} else {
							element.select2(opts);
						}
	...
	directives.directive('squareSelect2', select2);
