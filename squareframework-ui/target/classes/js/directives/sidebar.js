define(['square/directives/directives', 'text!square/views/sidebar.html'], function (directives, template) {
	'use strict';
	
	directives.directive('squareSidebar', function ($window,  $location, $webstorage) {
		...
		return {
			...
			link: function ($scope, element, attr) {
				...
				$scope.$watchCollection(attr.squareSidebar, function () {
					var items = $scope.$eval(attr.squareSidebar);
				...
				$scope.auto = $webstorage.local.get('square.sidebar.auto');
				...
				$scope.storage = function () {
					$webstorage.local.add('square.sidebar.auto',  $scope.auto);
				};
			}
		};
	});
