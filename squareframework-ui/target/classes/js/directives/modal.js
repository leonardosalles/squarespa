define(['square/directives/directives'], function (directives) {
	...
	directives.directive('squareModal', function ($parse) {
		return {
			restrict: 'A',
			...
			link: function (scope, element, attrs) {
				...
				var setClosed = function () {
					scope.$apply(function () {
						$parse(attrs.squareModalClose)(scope);
					});
				};
		        
				scope.$watch(attrs.squareModal, function () {


					var controlClose = $('<div class="control-close"></div>');
					if (!attrs.keyboard || attrs.keyboard === 'true') {
						controlClose.append($('<span>' + I18n.t('square.modalEsc') + '</span>'));
					}
					...
					var show = scope.$eval(attrs.squareModal);
