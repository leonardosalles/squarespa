define(['square/directives/directives', 'text!square/views/subheader.html'], function (directives, template) {
	'use strict';

	directives.directive('squareSubheader', function ($window, $location, $timeout, $filter) {
		...
		return {
			...
			link: function (scope, element, attrs) {
				...
				scope.$watchCollection(attrs.squareSubheader, function () {
					element.find('.navbar-inner > ul > li.dropdown').remove();

					var item = scope.$eval(attrs.squareSubheader);
			...
			controller: function ($scope) {
				...
				$scope.select2 = {
					...
					placeholder: I18n.t('square.subheader.quickaccess'),
